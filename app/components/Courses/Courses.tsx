...
const names: Name[] = [
  ...
  {
    course: "Penjumlahan",
    imageSrc: "/assets/courses/addition.png",
    profession: "Penjumlahan Tiga Angka",
    price: "0",
    category: "math",
  },
  {
    course: "Pengurangan",
    imageSrc: "/assets/courses/substraction.jpeg",
    profession: "Pengurangan Tiga Angka",
    price: "0",
    category: "math",
  },
  {
...
const NamesList = () => {
  ...
  const handleClick = (name: string) => {
    const targetMateri = name.toLowerCase(); // Pastikan kursus diubah ke huruf kecil
    router.push(`/materi?materi=${targetMateri}`); // Arahkan ke URL dengan query 'materi'
  };
  ...
  const math = names.filter((name) => name.category === "math");
  ...
  const nameElements = selectedNames.map((name, index) => (
    <div
      key={index}
      onClick={() => handleClick(name.course)}
    ...
    <div>
      ...
      >
        ...
        <div className="flex nowhitespace space-x-5 rounded-xl bg-white p-1 overflow-x-auto">
          {/* FOR DESKTOP VIEW */}
          <button
            onClick={() => setSelectedButton("math")}
            className={
              "bg-white " +
              (selectedButton === "math"
